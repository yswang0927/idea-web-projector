...
# yswang add
PROJECTOR_PROPS="-DIDE_HOME=${IDE_HOME} -DORG_JETBRAINS_PROJECTOR_SERVER_PORT=${port}"
SSL_FILE="${IDE_HOME}/bin/ssl.properties"
# 先检测外部环境变量是否传递了ssl配置文件
if [ -n "$IDE_SERVER_SSL_CONFIG_PATH" ]; then
  PROJECTOR_PROPS="${PROJECTOR_PROPS} -DORG_JETBRAINS_PROJECTOR_SERVER_SSL_PROPERTIES_PATH=${IDE_SERVER_SSL_CONFIG_PATH}"
